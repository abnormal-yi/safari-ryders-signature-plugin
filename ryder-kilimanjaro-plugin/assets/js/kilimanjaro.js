document.addEventListener('DOMContentLoaded', function() {
  document.querySelectorAll('.ryder-kili-wrapper').forEach(function(wrapper) {
    ...
    var map = L.map(mapEl.id, {center:[-3.07,37.20],zoom:10,zoomControl:true,scrollWheelZoom:false});
    ...
    wrapper.querySelectorAll('.mw-dc').forEach(function(dc) {
      dc.addEventListener('click', function() {
        var day = this.getAttribute('data-day');
        wrapper.querySelectorAll('.mw-dc').forEach(function(c){c.classList.remove('on');});
    ...
    wrapper.querySelectorAll('.faq-q').forEach(function(btn) {
    ...
    if (popupOverlay) {
      ...
      // Close popup via clicking outside
